...
| `R2_HEALTHCHECK_KEY` | Object key R2 probe reads to validate the bucket connection (F-OBS-07). | `apps/web/src/app/api/health/route.ts`, `packages/integrations/src/services/health-service.ts` | `_health/canary.txt` | **No (env var); but canary upload required for the probe to be meaningful** | The env var has a hardcoded fallback (`_health/canary.txt`) at `apps/web/src/app/api/health/route.ts:40` and the health service mirror, so the boot doesn't depend on it. The probe ALSO returns `ok` when the key 404s (`route.ts:223` — NotFound is treated as success), so a green `r2` probe is **not** proof of bucket wiring. Upload a canary file under the resolved key in **each region's** bucket before declaring the probe trustworthy. See §5.1 caveat. |
