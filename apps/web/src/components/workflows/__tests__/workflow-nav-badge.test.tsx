...
describe("WorkflowNavBadge", () => {
  it("renders nothing when count is 0", () => {
    mockedUseQuery.mockReturnValue({ data: { count: 0 } } as any);
    const { container } = render(<WorkflowNavBadge />);
    expect(container.innerHTML).toBe("");
  });

  it("renders count when there are overdue tasks", () => {
    mockedUseQuery.mockReturnValue({ data: { count: 5 } } as any);
    render(<WorkflowNavBadge />);
    expect(screen.getByText("5")).toBeInTheDocument();
  });

  it("renders 9+ when count exceeds 9", () => {
    mockedUseQuery.mockReturnValue({ data: { count: 15 } } as any);
    render(<WorkflowNavBadge />);
    expect(screen.getByText("9+")).toBeInTheDocument();
  });

  it("has aria-label for accessibility", () => {
    mockedUseQuery.mockReturnValue({ data: { count: 3 } } as any);
    render(<WorkflowNavBadge />);
