 ...
 */
export function ClassificationDisclaimerDialog(props: ClassificationDisclaimerDialogProps) {
  const { assessmentId, countryCode, open, onAcknowledged, onDeferred } = props;
  ...
  const handleConfirm = useCallback(() => {
    if (!acknowledged || ackMutation.isPending) return;
    ackMutation.mutate({ assessmentId });
  }, [ackMutation, acknowledged, assessmentId]);
  ...
  return (
    <AlertDialog open={open} onOpenChange={handleOpenChange}>
      <AlertDialogContent
        ...
        <AlertDialogFooter>
          <AlertDialogCancel onClick={onDeferred} disabled={ackMutation.isPending}>
            {t('disclaimer.defer')}
          </AlertDialogCancel>
          <AlertDialogAction
            onClick={handleConfirm}
            disabled={!acknowledged || ackMutation.isPending}>
            {ackMutation.isPending ? t('disclaimer.acknowledging') : t('disclaimer.confirm')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
