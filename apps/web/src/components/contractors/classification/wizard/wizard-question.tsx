...
import {
  LikertAnswer,
  type LikertValue,
} from './answers/likert-answer';
import {
  Score03Answer,
  type Score03Value,
} from './answers/score-03-answer';
import {
  YesNoAnswer,
  type YesNoValue,
} from './answers/yes-no-answer';
import {
  EconomicDependencyInput,
} from './schein/economic-dependency-input';
...
export function WizardQuestion({
  ...
  const legalKind: 'case-law' | 'drv' =
    question.caseLawCitation != null ? 'case-law' : 'drv';
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border/60 bg-surface-1 p-4 shadow-sm">
      <div className="flex flex-col gap-2">
        <p id={promptId} className="text-sm font-medium text-foreground">
          {prompt}
          {question.required ? (
            <span aria-hidden="true" className="ms-1 text-destructive">*</span>
          ) : null}
        </p>
    ...
    case 'likert-5':
      return (
        <LikertAnswer
          ...
          onChange={v => onChange({ type: 'likert-5', value: v })}
          disabled={disabled}
          aria-labelledby={promptId}
        />
      );
    case 'score-0-3':
      return (
        <Score03Answer
          ...
          onChange={v => onChange({ type: 'score-0-3', value: v })}
          disabled={disabled}
          aria-labelledby={promptId}
        />
      );
    case 'billing-ratio':
      return (
        <EconomicDependencyInput
          ...
          onCommit={v => onChange({ type: 'billing-ratio', value: v })}
          disabled={disabled}
        />
      );
    case 'rationale':
