...
import { SteuernummerInput } from '@/components/contractors/compliance/steuernummer-input';
...
describe('SteuernummerInput', () => {
  ...
  it('shows format hint for the selected Bundesland', () => {
    render(<SteuernummerInput {...defaultProps} bundesland="BW" />);
    expect(screen.getByText(/Format: 93\/815\/08152/)).toBeInTheDocument();
  ...
  it('shows local validation error on blur with an invalid value', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(/does not match Baden-Württemberg/);
  });
