...
import { queryOptions, trpcQueryKey } from '@/test/mocks/trpc';
import { render, screen } from '@/test/test-utils';
import { VatRateSelector } from '../vat-rate-selector';

function rqKey() {
  return '__rq_vat_rate_selector__';
}
function rq() {
...
vi.mock('@/trpc/init', () => ({
  trpc: {
    tax: {
      getRates: {
        queryOptions: () => queryOptions(trpcQueryKey(['tax', 'getRates'])),
      },
    },
  },
}));
...
describe('VatRateSelector', () => {
  beforeEach(() => {
    rq().setQueryResult(trpcQueryKey(['tax', 'getRates']), {
      isLoading: false,
      data: [
        {
          id: '1',
