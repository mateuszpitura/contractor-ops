...
import type { OrgSwitcherOption } from './use-org-switcher';
...
interface OrgSwitcherListProps {
  ...
  /**
   * `'menu'` renders rows as `role="menuitemradio"` for use inside a
   * DropdownMenu submenu. `'sheet'` renders rows as buttons for use inside
   * the mobile sheet, where ARIA menu semantics would be wrong.
   */
  variant: 'menu' | 'sheet';
}

/**
 * Org-switcher row list shared between the desktop dropdown submenu and
 * the mobile sheet. Renders org logo (or initial fallback), name, an
 * inline checkmark on the active org, and a spinner on the row being
 * switched to.
 */
