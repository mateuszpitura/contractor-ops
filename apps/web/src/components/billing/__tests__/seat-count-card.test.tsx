...
describe('SeatCountCard', () => {
  it('renders the active seats label and count', () => {
    render(<SeatCountCard activeContractors={5} includedSeats={10} seatPriceMinor={1500} />);
  ...
  it('shows active/included text', () => {
    render(<SeatCountCard activeContractors={5} includedSeats={10} seatPriceMinor={1500} />);
    expect(screen.getByText('5 active / 10 included')).toBeInTheDocument();
  });
  ...
  it('shows overage text when active exceeds included', () => {
    render(<SeatCountCard activeContractors={12} includedSeats={10} seatPriceMinor={1500} />);
    expect(screen.getByText('2 additional seats billed at 15/seat')).toBeInTheDocument();
  });
