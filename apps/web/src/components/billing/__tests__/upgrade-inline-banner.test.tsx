...
describe("UpgradeInlineBanner", () => {
  it("renders with status role and aria-live", () => {
    render(
      <UpgradeInlineBanner featureName="OCR" requiredTier="Pro" />,
    );
  ...
  it("displays the upgrade button with link to billing", () => {
    render(
      <UpgradeInlineBanner featureName="API access" requiredTier="Enterprise" />,
    );
  ...
  it("shows correct message for Enterprise tier", () => {
    render(
      <UpgradeInlineBanner
        featureName="Audit log export"
        requiredTier="Enterprise"
      />,
    );
    expect(
      screen.getByText("Audit log export requires Enterprise."),
    ).toBeInTheDocument();
  });
});
