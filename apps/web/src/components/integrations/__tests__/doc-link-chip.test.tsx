...
describe("DocLinkChip", () => {
  ...
  it("renders a link pointing to the url", () => {
    ...
    const link = screen.getByLabelText(
      "Open Design Doc in Notion (new tab)",
    );
  ...
  it("does not show remove button in readOnly mode", () => {
    ...
    render(
      <DocLinkChip {...baseProps} readOnly={true} onRemove={onRemove} />,
    );
    expect(
      screen.queryByLabelText("Remove link to Design Doc"),
  ...
  it("shows remove button when not readOnly and onRemove provided", () => {
    ...
    expect(
      screen.getByLabelText("Remove link to Design Doc"),
    ).toBeInTheDocument();
  });
  ...
  it("opens confirmation dialog when remove button clicked", async () => {
    const onRemove = vi.fn();
    const { user } = setup(
      <DocLinkChip {...baseProps} onRemove={onRemove} />,
    );
