...
const validationPillClass: Record<string, string> = {
  ...
  WARNINGS: 'bg-amber-500/10 text-amber-600 dark:text-amber-400',
...
function TransmissionRow({
  ...
  t: ReturnType<typeof useTranslations<'EInvoice.TransmissionsLog'>>;
}) {
      ...
      <TableCell>
        {validation ? (
          <Badge variant="secondary" className={validationPillClass[validation] ?? ''}>
            {t(`validation.${validation}` as Parameters<typeof t>[0])}
          </Badge>
        ) : (
      ...
      <TableCell>
        {transmission ? (
          <Badge variant="secondary" className={transmissionPillClass[transmission] ?? ''}>
            {t(`transmission.${transmission}` as Parameters<typeof t>[0])}
          </Badge>
        ) : (
...
export function TransmissionsLogCard() {
  ...
  const listQuery = useInfiniteQuery({
    ...trpc.einvoice.listByOrg.infiniteQueryOptions(
      // `all` means "no filter" — keep the wire shape and skip the field.
