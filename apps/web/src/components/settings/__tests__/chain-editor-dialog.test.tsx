...
describe("ChainEditorDialog", () => {
  ...
  it("renders create description text in create mode", () => {
    ...
    expect(
      screen.getByText(/Set up a new approval chain/i),
    ).toBeInTheDocument();
  });
  ...
  it("does not render when closed", () => {
    ...
    expect(
      screen.queryByText("Create approval chain"),
  ...
  it("shows remove level button when more than one level exists", async () => {
    ...
    expect(
      screen.queryByLabelText("Remove level"),
    ).not.toBeInTheDocument();
    // Add second level
    await user.click(screen.getByText("Add level"));
  ...
  it("renders edit description text in edit mode", () => {
    ...
    expect(
      screen.getByText(/Update chain settings/i),
    ).toBeInTheDocument();
  });
  ...
  it("populates multiple steps when editing chain with two steps", () => {
    ...
    expect(
      screen.getByDisplayValue("Manager Review"),
    ).toBeInTheDocument();
    expect(
      screen.getByDisplayValue("Finance Review"),
    ).toBeInTheDocument();
  ...
  it("shows zero conditions when editing chain has empty conditions", () => {
    ...
    render(
      <ChainEditorDialog
        open={true}
        onOpenChange={vi.fn()}
        chainData={chainWithNoConditions}
      />,
    );
    expect(screen.getByTestId("condition-count")).toHaveTextContent("0");
  });
