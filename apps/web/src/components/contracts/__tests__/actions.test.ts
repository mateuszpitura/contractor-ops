...
import type { ContractRowLike, ContractStatus } from '../actions';
import {
...
describe('contract actions registry', () => {
  ...
  describe('getBulkContractActions', () => {
    ...
    it('bulk terminate is marked destructive', () => {
      const t = getBulkContractActions().find(a => a.key === 'bulk.terminate');
  ...
  describe('getDetailContractActions — status filtering', () => {
    ...
    it('row with unknown status hides all stage-gated actions but keeps unconditional ones', () => {
