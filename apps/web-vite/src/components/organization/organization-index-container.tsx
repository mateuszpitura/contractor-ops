import { Building2, Layers, Wallet } from 'lucide-react';
import { useTranslations } from '../../i18n/useTranslations.js';
import { useOrganizationIndex } from './hooks/use-organization-index.js';
import type { SummaryCardItem } from './organization-index-view.js';
import { OrganizationIndexSkeleton, OrganizationIndexView } from './organization-index-view.js';
import { OrganizationLayout } from './organization-layout.js';

export function OrganizationIndexContainer() {
  const t = useTranslations('Organization');
  const { isLoading, teamsCount, projectsCount, costCentersCount } = useOrganizationIndex();

  if (isLoading) {
    return (
      <OrganizationLayout>
        <OrganizationIndexSkeleton />
      </OrganizationLayout>
    );
  }

  const items: SummaryCardItem[] = [
    { href: '/organization/teams', label: t('summaryTeams'), count: teamsCount, icon: Building2 },
    {
      href: '/organization/projects',
      label: t('summaryProjects'),
      count: projectsCount,
      icon: Layers,
    },
    {
      href: '/organization/cost-centers',
      label: t('summaryCostCenters'),
      count: costCentersCount,
      icon: Wallet,
    },
  ];

  return (
    <OrganizationLayout>
      <OrganizationIndexView items={items} />
    </OrganizationLayout>
  );
}
