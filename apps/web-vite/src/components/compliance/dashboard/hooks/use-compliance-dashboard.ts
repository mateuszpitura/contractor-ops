...
/** Blocked-payments tab refreshes every 60s (paused on backgrounded tab by default). */
const BLOCKED_PAYMENTS_POLL_MS = 60_000;

export type ComplianceDashboardTab = 'at-risk' | 'upcoming-renewals' | 'blocked-payments';

/**
 * The ONLY tRPC boundary for the admin compliance dashboard. Fetches the KPI
 * counts + the three list endpoints in parallel and returns a flags-and-props
 * bag. The container decides loading/empty/error; this hook never branches on them.
 */
