/**
 * web-vite port. `createMockTranslator` was not lifted into web-vite; a
 * thin echo-translator mirrors its semantics for cell-renderer tests.
 */
...
vi.mock('date-fns', async importOriginal => {
  ...
  return { ...actual, formatDistanceToNow: () => '2 days ago' };
});

vi.mock('../../compliance-health-badge.js', () => ({
...
const mockT: LooseTranslator = (key: unknown) => String(key);
...
function renderCell(columnId: string, row: ContractorRow) {
  ...
  const col = columns.find(
    c =>
      ('accessorKey' in c && (c as { accessorKey: string }).accessorKey === columnId) ||
      c.id === columnId,
  );
...
describe('getColumns cell renderers', () => {
  ...
  it('owner cell renders name when owner exists', () => {
    renderCell(
      'owner',
      makeRow({
