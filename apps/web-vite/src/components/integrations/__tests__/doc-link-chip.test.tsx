...
// ---------------------------------------------------------------------------
// Mocks — tooltip wraps an `<a>` via `render` prop; alert-dialog mock unmounts
// while closed so we can assert open/close transitions.
// ---------------------------------------------------------------------------
...
vi.mock('@contractor-ops/ui/components/shadcn/tooltip', () => ({
  ...
  TooltipTrigger: ({
    ...
    if (renderProp) {
      const { props } = renderProp as ReactElement<Record<string, unknown>>;
      return <a {...(props as AnchorHTMLAttributes<HTMLAnchorElement>)}>{children}</a>;
    }
