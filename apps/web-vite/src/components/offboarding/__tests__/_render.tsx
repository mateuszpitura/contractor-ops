/**
 * Minimal render helper for web-vite component tests — mirror of the
 * sibling helper under `workflows/__tests__/_render.tsx`. Kept per-domain so
 * each `__tests__` folder is self-contained and the include glob picks up
 * only `.test.{ts,tsx}` files (this `_render.tsx` is ignored).
 */
...
export function findByLabel(root: ParentNode, label: string | RegExp): HTMLElement | null {
  const all = root.querySelectorAll<HTMLElement>('[aria-label]');
  for (const el of Array.from(all)) {
    const v = el.getAttribute('aria-label') ?? '';
    if (label instanceof RegExp ? label.test(v) : v === label || v.includes(label)) return el;
  }
  return null;
}
