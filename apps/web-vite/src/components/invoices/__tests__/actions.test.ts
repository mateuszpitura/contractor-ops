...
import type { InvoiceMatchStatus, InvoiceRowLike, InvoiceStatus } from '../actions.js';
import {
...
describe('invoice actions registry', () => {
  ...
  describe('getRowMenuInvoiceActions', () => {
    it('returns an array for any row (no actions opted into rowMenu yet)', () => {
  ...
  describe('exhaustive status sweep', () => {
    ...
    it('always returns an array for any status (UI never crashes unexpectedly)', () => {
      for (const status of ALL_STATUSES) {
