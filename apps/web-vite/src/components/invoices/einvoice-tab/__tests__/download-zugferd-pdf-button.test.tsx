...
describe('DownloadZugferdPdfButton', () => {
  it('renders the download CTA when idle', () => {
    render(<DownloadZugferdPdfButton onDownload={vi.fn()} isPending={false} />);
  ...
  it('shows the generating-label and disables the button while pending', () => {
    render(<DownloadZugferdPdfButton onDownload={vi.fn()} isPending />);
  ...
  it('invokes onDownload when clicked', async () => {
    ...
    const { user } = setup(<DownloadZugferdPdfButton onDownload={onDownload} isPending={false} />);
