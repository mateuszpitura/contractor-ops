...
import { usePortalSigningUrl } from './use-portal-settings.js';
...
export function usePortalEmbeddedSigningModal(
  envelopeId: string,
  ...
  const signingUrlQuery = usePortalSigningUrl(
    { envelopeId, recipientEmail, returnUrl },
    { enabled: open && !!envelopeId && !!returnUrl, usePortalAuth },
  );
