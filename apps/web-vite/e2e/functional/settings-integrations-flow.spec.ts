...
/**
 * Settings — Integrations flow — Step 13 port from
 * apps/web/e2e/functional/settings-integrations-flow.spec.ts.
 *
 * Routes: /:locale/settings?tab=integrations (Vite SPA, default locale pl).
 * OAuth provider redirects are now served by Fastify on apps/api
 * (`/api/oauth/:provider/start`, default http://localhost:4000) — the SPA
 * `Connect` handlers either open a configuration dialog or navigate to that
 * host. Assertions here only check for redirect or dialog presence, not the
 * destination origin, matching legacy intent.
 */
test.describe('Settings — Integrations flow', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToDashboard(page, `/${E2E_LOCALE}/settings?tab=integrations`);
    skipIfUnauthenticated(page);
  });

  test('integrations tab content is visible', async ({ page }) => {
    const integrationsTab = page.locator('[role="tab"]', {
      hasText: /integracje|integrations/i,
    });
  ...
  test('integration cards show connection status', async ({ page }) => {
    ...
    const statusBadge = page
      ...
      .filter({
        hasText:
          /connected|disconnected|active|inactive|enabled|disabled|połączon|rozłączon|aktywn|nieaktywn|włączon|wyłączon/i,
      })
      .first();
  ...
  test('connect button visible on disconnected integrations', async ({ page }) => {
    ...
    const connectButton = page
      .locator('button, a')
      .filter({
        hasText: /^connect$|connect integration|set up|configure|^połącz$|skonfiguruj|ustaw/i,
      })
      .first();
