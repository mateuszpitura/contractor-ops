...
/**
 * Functional-suite accessibility smoke — Step 13 port from
 * apps/web/e2e/functional/accessibility.spec.ts.
 *
 * Distinct from the dedicated WCAG gate in `apps/web-vite/e2e/a11y/`:
 * this suite uses Playwright built-in assertions to verify landmarks,
 * headings, focus order, labels, and the skip-to-content link on the
 * dashboard root and the login page.
 *
 * Web-vite shape notes:
 * - Dashboard index is the locale root (e.g. `/pl`), not `/pl/dashboard`.
 * - Login form uses `useId()`, so input ids are dynamic — labels are
 *   matched by suffix (`label[for$="-email"]`) rather than `for="email"`.
 */

test.describe('Accessibility — Dashboard', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToDashboard(page, `/${E2E_LOCALE}`);
...
test.describe('Accessibility — Login page', () => {
  ...
  test('form inputs have associated labels', async ({ page }) => {
    ...
    // The login form uses `useId()`, so input ids are dynamic. Match the
    // email input by type, then verify a `<label for=...>` references it.
    const emailInput = page.locator('input[type="email"]').first();
    await emailInput.waitFor({ state: 'visible', timeout: 15_000 });

    const emailId = await emailInput.getAttribute('id');
    expect(emailId, 'email input has an id').not.toBeNull();
    ...
    expect(passwordId, 'password input has an id').not.toBeNull();
