...
/**
 * WCAG 2.2 AA accessibility gate for the contractor portal — Vite SPA port.
 *
 ...
 *
 * Auth: depends on the portal session storage state produced by global
 * setup. When the cookie is absent we land on `/portal/login` and skip
 * the scan — same pattern as the dashboard gate.
 *
 * Web-vite note: the legacy `#portal-content` id does not exist in the
 * new SPA; we wait for the first `main` element to become visible
 * instead, since every portal page renders content inside `main`.
 */
