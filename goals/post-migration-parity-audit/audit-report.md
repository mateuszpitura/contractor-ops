...
| OBSERVABILITY | 0 / 2 / 0 (GAP-OBSERVABILITY-007 cron-worker + -008 public-api inline-fixed) | 2 / 1 / 0 (GAP-OBSERVABILITY-003 inline-fixed) | 2 / 1 / 0 (GAP-OBSERVABILITY-006 inline-fixed) |
...
`PII_KEYWORDS` list = byte-for-byte port (26 → 26 + 26 + 26 + 26 entries across apps/api, apps/web-vite, apps/cron-worker, apps/public-api). All nine scrub branches preserved (`user.email`, `user.ip_address → {{auto}}`, `request.{data,query_string,headers,cookies}`, `extra`, `contexts`, `tags`, `breadcrumbs[].data`), `MAX_DEPTH = 6`, `maskEmail()` unchanged. `beforeSend: scrubSentryEvent` wired on the **backend Fastify API** (`apps/api/src/lib/sentry.ts:38`), the **SPA** (`apps/web-vite/src/sentry.ts:31`), the **cron-worker** (`apps/cron-worker/src/lib/sentry.ts` — post GAP-OBSERVABILITY-007 inline fix), and the **public REST API** (`apps/public-api/src/lib/sentry.ts` — post GAP-OBSERVABILITY-008 inline fix). Every Sentry runtime in the monorepo now redacts PII at the `beforeSend` boundary. No `beforeSendTransaction` in legacy → no parity gap.
