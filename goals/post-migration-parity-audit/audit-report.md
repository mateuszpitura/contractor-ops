...
| ID | Surface | New path | Sev | Evidence | Status | Remediation |
|----|---------|----------|-----|----------|--------|-------------|
| GAP-SECURITY-001 | SPA CSP `script-src` | `render.yaml:684` + `apps/web-vite/index.html:28` | **P0** | New `script-src 'self' 'wasm-unsafe-eval' https://*.sentry-cdn.com https://challenges.cloudflare.com` replaces legacy `'self' 'nonce-${nonce}' 'strict-dynamic' …`. Loses XSS containment — any same-origin script injection executes unconditionally. Legacy: `apps/web/src/middleware.ts:610-625`. | open | Render Static-Site does not support per-request nonces. Options: (a) move SPA serving behind an SSR/edge worker that mints a nonce (e.g. Cloudflare Worker, Fastify static plugin with per-request CSP), (b) tighten with explicit hash pinning of every inline `<script>` chunk + remove `'wasm-unsafe-eval'` if not required, (c) accept the risk with a written design-review note. Restoration agent owns the decision. |
| GAP-SECURITY-002 | SPA CSP `frame-src` | `render.yaml:684` | **P0** | Legacy: 3 hosts (`*.docusign.com`, `*.docusign.net`, `apps-d.docusign.com`). New: 6 hosts (adds `*.autenti.com`, `*.r2.cloudflarestorage.com`, `challenges.cloudflare.com`). `*.r2.cloudflarestorage.com` wildcard in `frame-src` is unusually broad; mitigated by `frame-ancestors 'none'` (same line). | open | Verify intake-detail PDF iframe ships `sandbox=`; consider narrowing R2 host to the specific bucket subdomain (e.g. `https://contractor-ops-prod.r2.cloudflarestorage.com`). |
