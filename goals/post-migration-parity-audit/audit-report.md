...
| TEST | **0 / 3 / 0** (GAP-TEST-015 / -021 / -026 all inline-fixed — payment idempotency `f75c18a6`, cross-org IDOR `59173034`, cross-jurisdiction IDOR `140e865b`) | 6 / 5 / 0 (GAP-TEST-001/002 inline-fixed under GAP-WEBHOOK-003; GAP-TEST-016 + -017 inline-fixed `1d4aa43c`; GAP-TEST-018 inline-fixed `c767b561`) | 12 / 0 / 0 |
| **Total** | **4 / 7 / 0** | **6 / 11 / 14** (+1 closed-verified: GAP-SECURITY-004) | **13 / 10 / 9** (+3 closed-verified: GAP-SECURITY-007, GAP-MIDDLEWARE-001, GAP-OBSERVABILITY-013) |
...
| GAP-OBSERVABILITY-012 | PostHog `signup_completed` / `alias` / `identify` post-signup hook never shipped | `packages/auth/src/config.ts` `databaseHooks.user.create.after` | **P0** | Restored from `.audit-scratch/observability/section.md` GAP-OBSERVABILITY-010 (re-imported by calibration; severity preserved per spec — P0 conversion analytics). `goals/launch-readiness-landing-pricing-analytics/plan.md:122` specifies the `packages/auth` post-signup hook firing `posthog.capture('signup_completed', { userId })` + `posthog.alias()` to stitch anonymous landing distinct_id → userId. No such call exists in current `packages/auth/src/` or anywhere under `apps/`. Conversion-funnel analytics (anonymous landing visitor → paid signup) is lost — directly affects launch-readiness analytics planned for landing. | **open (escalated)** | Blocker: implementation needs (a) add `posthog-node` to `packages/auth` deps + (b) add `POSTHOG_PROJECT_API_KEY` to the api/cron-worker/public-api env schemas + (c) wire the capture + alias calls inside the existing `databaseHooks.user.create.after` at `packages/auth/src/config.ts:290` + (d) read the anonymous distinct id from the PostHog cookie on the signup request to feed `alias({ alias: anonId })`. Product owner must also confirm the event payload schema (`signup_completed` + properties) so it matches the landing-side capture from `goals/launch-readiness-landing-pricing-analytics/plan.md:122`. Pairs with that launch-readiness work item — recommend implementing both in one cohesive change rather than partial-shipping here. |
