/**
 * Non-tenant-scoped Prisma client for cross-org aggregates.
 *
 * ONLY for cron cross-org reads (economic-dependency scan — the billing-share
 * denominator needs to aggregate a contractor's invoices across ALL
 * organisations they bill, not just the current tenant).
 *
 ...
 * every call-site must be auditable. Grep for `prismaRaw` to list every
 * deliberate cross-org read.
 */
...
// Singleton raw client — no tenant extension.
