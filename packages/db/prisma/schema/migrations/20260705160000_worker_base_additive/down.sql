...
-- Contractor.workerId unique index, drop the column, drop the Worker tenant FK,
-- drop the Worker table and its enum type. Contractor rows are never touched
-- destructively; only the added column is removed and any (orphaned) Worker rows
-- go with the table.
--
