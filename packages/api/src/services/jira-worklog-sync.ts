...
// Use loosely typed prisma client — TimeEntry/Timesheet models may be
// generated by a parallel plan (18-01) and not yet available in this build.
  ...
  try {
    ...
    while (true) {
      ...
      if (!searchResponse.ok) {
        ...
        throw new Error(
          `Jira search API error (${searchResponse.status}): ${text}`,
        );
      }

      const searchData =
        (await searchResponse.json()) as JiraSearchResponse;
    ...
    for (const issue of allIssues) {
      ...
      while (true) {
        const worklogUrl = new URL(
          `${baseUrl}/issue/${issue.key}/worklog`,
        );
        worklogUrl.searchParams.set("startAt", String(worklogStartAt));
        worklogUrl.searchParams.set(
          "maxResults",
          String(worklogMaxResults),
        );
        ...
        const worklogData =
          (await worklogResponse.json()) as JiraWorklogResponse;
        ...
        for (const worklog of userWorklogs) {
          ...
          const description =
            commentText ?? `${issue.key}: ${issue.fields.summary}`;
        ...
        if (
          worklogStartAt + worklogData.worklogs.length >=
          worklogData.total
        )
          break;
