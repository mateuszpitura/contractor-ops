...
import {
  CLOCKIFY_REGIONS,
  type ClockifyRegion,
} from "@contractor-ops/integrations/adapters/clockify-adapter";
...
export async function syncClockifyEntries(
  ...
  if (!config?.workspaceId || !config?.userId) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "Clockify connection is missing workspaceId or userId in config",
    });
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Clockify API error (${response.status}): ${text}`,
        );
      }
