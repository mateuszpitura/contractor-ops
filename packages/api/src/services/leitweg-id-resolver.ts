...
//
// Phase 61 · Plan 61-04 · EINV-05 — resolve the effective Leitweg-ID for an
// invoice per CONTEXT D-06:
//
...
//   3. Else → return `null`. Callers (Plan 06 `finalizeEInvoice`) treat this
//      as a soft-gate: the XRechnung XML can still be generated without BT-10
//      for non-Leitweg-mandated buyers, while the UI flags the invoice red.
//
// Invariants:
// - Every query is scoped by `organizationId`. Cross-tenant leakage is
//   impossible at this layer regardless of the caller's Prisma extension
//   state.
// - Two short, tier-specific queries (never a single OR) so each DB plan is
//   trivially readable and each short-circuits as soon as the upstream tier
//   hits.
...
export interface ResolvedLeitwegId {
  ...
  value: string;
  /** Which D-06 tier produced the hit — surfaced on the finalize response for auditing. */
...
/**
 * Resolve the effective Leitweg-ID for an invoice per CONTEXT D-06.
 *
...
export async function resolveLeitwegIdForInvoice(
  ...
  // Tier 1 — contract-scoped override (D-06 rule 1). Skip entirely when the
  ...
  // Tier 2 — contractor default (D-06 rule 2). Must filter explicitly by
  ...
  // Tier 3 — D-06 rule 3: no match → soft-gate.
  return null;
}
