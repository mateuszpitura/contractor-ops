...
// Phase 78 IDP-07 — GitHub per-provider connection-lifecycle router.
//
// The per-org ENABLE toggle is owned by `deprovisioning.enableProviderForOrg`
// (extended in 78-06); this router exposes the GitHub-scoped status + a thin
...
const FLAG_KEY = 'module.idp-deprovisioning-github';
...
export const githubRouter = router({
  /** Per-org GitHub deprovisioning status: signoff approval + current enabled flag. */
  ...
  /**
   * Enable/disable GitHub deprovisioning for this org. GATE: refuses to enable
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await writeAuditLog({
        ...
        action: input.enabled
          ? 'idp.github.deprovisioning_enabled'
          : 'idp.github.deprovisioning_disabled',
