...
const log = createLogger({ service: 'equipment-returns-router' });
...
export const equipmentReturnsRouter = router({
  ...
  approveReturnRequest: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      }).catch(err => log.error({ err }, 'failed to dispatch return approval notification'));
  ...
  rejectReturnRequest: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      }).catch(err => log.error({ err }, 'failed to dispatch return rejection notification'));
