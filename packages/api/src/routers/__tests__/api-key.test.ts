/**
 * API Key router unit tests.
 *
 * Strategy:
 *  - Mock `@contractor-ops/db` with a vi.hoisted mockPrisma.
 *  - Mock `@contractor-ops/auth`, api-key-service, logger, Sentry.
...
// ---------------------------------------------------------------------------
// Mock Prisma + service (hoisted)
// ---------------------------------------------------------------------------

const { mockPrisma, mockGenerateApiKey } = vi.hoisted(() => {
...
describe('apiKey.create', () => {
  it('returns key with plaintext on successful creation', async () => {
    const createdKey = {
      id: 'key-1',
      name: 'My Key',
      prefix: 'co_live_abc',
    ...
    mockPrisma.organizationApiKey.create.mockResolvedValueOnce(createdKey);
    ...
    expect(mockPrisma.organizationApiKey.create).toHaveBeenCalledWith(
...
describe('apiKey.list', () => {
  it('returns all keys for the organization', async () => {
    ...
    expect(mockPrisma.organizationApiKey.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
...
describe('apiKey.update', () => {
  it('updates name and scopes for an active key', async () => {
    const existing = { id: 'key-1', organizationId: ORG_ID, revokedAt: null };
    const updated = {
      id: 'key-1',
      name: 'Updated Name',
      prefix: 'co_a',
    ...
    mockPrisma.organizationApiKey.findFirst.mockResolvedValueOnce(existing);
    mockPrisma.organizationApiKey.update.mockResolvedValueOnce(updated);
...
describe('apiKey.revoke', () => {
  it('revokes an active key', async () => {
    ...
    mockPrisma.organizationApiKey.update.mockResolvedValueOnce({});

    const result = await caller.apiKey.revoke({ id: 'key-1' });
