...
vi.mock('../../services/exchange-rate.js', () => ({
  getRate: mockGetRate,
...
describe('exchangeRate.query', () => {
  it('returns formatted exchange rates for a date range', async () => {
    const date = new Date('2026-03-15');
    mockPrisma.exchangeRate.findMany.mockResolvedValueOnce([
      { date, base: 'EUR', target: 'PLN', rate: 4.35, source: 'ECB' },
      { date: new Date('2026-03-14'), base: 'EUR', target: 'PLN', rate: 4.34, source: 'ECB' },
    ]);

    const result = await caller.exchangeRate.query({
      base: 'EUR',
      target: 'PLN',
...
describe('exchangeRate.latest', () => {
  it('returns the latest rate for a currency pair', async () => {
    mockGetRate.mockResolvedValueOnce({
      date: new Date('2026-03-15'),
      rate: 4.35,
      source: 'ECB',
    });

    const result = await caller.exchangeRate.latest({
      base: 'EUR',
      target: 'PLN',
    });
...
describe('exchangeRate.convert', () => {
  it('returns converted amount', async () => {
    mockConvertAmount.mockResolvedValueOnce({
      convertedAmountMinor: 43500,
      rate: 4.35,
      source: 'ECB',
    });

    const result = await caller.exchangeRate.convert({
      amountMinor: 10000,
      from: 'EUR',
      to: 'PLN',
    });
  ...
  it('throws when conversion is not possible', async () => {
    mockConvertAmount.mockResolvedValueOnce(null);

    await expect(
      caller.exchangeRate.convert({ amountMinor: 10000, from: 'EUR', to: 'XYZ' }),
    ).rejects.toThrow('Cannot convert EUR to XYZ');
  });
});
