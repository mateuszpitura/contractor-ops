...
export const webhookSubscriptionRouter = router({
  ...
  create: webhookAdminProcedure.input(createInput).mutation(async ({ ctx, input }) => {
    ...
    await writeAuditLog({
      ...
      newValues: { url: created.url, eventFilter: created.eventFilter, includePii: created.includePii },
  ...
  update: webhookAdminProcedure.input(updateInput).mutation(async ({ ctx, input }) => {
    ...
    await writeAuditLog({
      ...
      oldValues: { url: existing.url, eventFilter: existing.eventFilter, enabled: existing.enabled },
