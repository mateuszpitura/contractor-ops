...
export const equipmentCouriersRouter = router({
  ...
  createInPostShipment: tenantProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const shipments = await auditedMutation(
        auditMutationCtx(ctx),
        {
          action: 'shipment.createInPost',
          resourceType: 'SHIPMENT',
          resourceId: shipmentRows[0]?.id ?? '',
          newValues: {
  ...
  getShipmentLabel: tenantProcedure
    ...
    .query(async ({ ctx, input }) => {
      const shipment = await findOrThrow(
        () =>
          ctx.db.shipment.findFirst({
            where: {
              id: input.shipmentId,
              organizationId: ctx.organizationId,
            },
