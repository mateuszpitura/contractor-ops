// Slack org-grid minimum-privilege deprovision scopes.
//
// Single source of truth: the `lint:scopes` CI guard verifies that the
// SlackAdapter's `getOAuthConfig().scopes` array (for the SLACK_ORG_GRID
// connection) contains every scope listed here, and rejects any scope literal
// added directly to the adapter without going through this constant. Mirrors
// the GWS scope-const sibling.
//
