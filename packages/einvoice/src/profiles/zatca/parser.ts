...
export function parseZatcaXml(xml: string, metadata?: Record<string, unknown>): EInvoice {
  ...
  const refs = Array.isArray(additionalRefs) ? additionalRefs : [additionalRefs];
  ...
  for (const ref of refs) {
    ...
    if (refId === 'ICV') {
      ...
      const encoded = textOf(ref['cac:Attachment']?.['cbc:EmbeddedDocumentBinaryObject']);
      if (encoded) {
...
function parseParty(partyWrapper: Record<string, unknown>): EInvoiceParty {
  ...
  const taxScheme = party['cac:PartyTaxScheme'] as Record<string, unknown> | undefined;
  const legalEntity = party['cac:PartyLegalEntity'] as Record<string, unknown> | undefined;
  const postalAddress = party['cac:PostalAddress'] as Record<string, unknown> | undefined;
  ...
  const countryNode = postalAddress?.['cac:Country'] as Record<string, unknown> | undefined;
...
function parseLine(line: Record<string, unknown>): EInvoiceLine {
  ...
  const unit = (line['cbc:InvoicedQuantity'] as Record<string, unknown>)?.['@_unitCode'] as
    | string
    | undefined;
  ...
  const taxCategory = item?.['cac:ClassifiedTaxCategory'] as Record<string, unknown> | undefined;
  ...
  const vatAmountMinor = lineTax ? toMinorUnits(textOf(lineTax['cbc:TaxAmount'])) : undefined;
...
function parseTaxSubtotal(sub: Record<string, unknown>): EInvoiceTaxSubtotal {
  ...
  const category = sub['cac:TaxCategory'] as Record<string, unknown> | undefined;
