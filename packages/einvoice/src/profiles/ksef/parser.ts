...
import { ksefParsedInvoiceSchema, type KsefParsedInvoice } from "./schemas.js";
...
export function parseFa3Xml(
  ...
  // Seller
  const sellerIdent = (podmiot1["DaneIdentyfikacyjne"] ?? {}) as Record<
    string,
    unknown
  >;
  const sellerAddress = podmiot1["Adres"] as
  ...
  // Buyer
  const buyerIdent = (podmiot2["DaneIdentyfikacyjne"] ?? {}) as Record<
    string,
    unknown
  >;
  ...
  const linesArray = Array.isArray(rawLines)
    ? rawLines
    : rawLines
      ? [rawLines]
      : [];
  ...
  const lines = linesArray.map((line) => {
    ...
    const vatRateStr =
      line["P_12"] != null ? String(line["P_12"]) : undefined;
    const vatAmount =
      line["P_11A"] != null
    ...
    return {
      ...
      unitPriceMinor:
        line["P_9A"] != null ? toMinorUnits(line["P_9A"]) : undefined,
  ...
  const payment = platnosc
    ? {
        dueDate:
          platnosc["TerminPlatnosci"] != null
            ? String(platnosc["TerminPlatnosci"])
            : undefined,
        bankAccount:
          (platnosc["NrRB"] ??
            dig(
              platnosc as Record<string, unknown>,
              "RachunekBankowy",
              "NrRB",
            )) != null
            ? String(
            ...
            : undefined,
        method:
          platnosc["FormaPlatnosci"] != null
            ? String(platnosc["FormaPlatnosci"])
            : undefined,
      }
    : undefined;
  ...
  const mapped = {
    ...
    seller: {
      ...
      name: String(
        sellerIdent["Nazwa"] ?? sellerIdent["PelnaNazwa"] ?? "",
      ),
    ...
    buyer: {
      ...
      name: String(
        buyerIdent["Nazwa"] ?? buyerIdent["PelnaNazwa"] ?? "",
      ),
    },
    lines,
    totals: {
