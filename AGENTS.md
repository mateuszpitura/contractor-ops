...
> Binding floor: [`.Codex/core-values.yml`](.Codex/core-values.yml) + [`.cursor/rules/`](.cursor/rules/). **Read this entire file** at session start and before non-trivial work. Summary below is a floor, not a replacement.

## Enforcement

- **Codex:** `SessionStart` injects `core-values.yml` + **full text of this file** on `startup`, `resume`, `/clear`, `compact` — that *is* context loaded; no Read tool needed unless the `PROJECT STANDARDS — AGENTS.md` block is missing. `UserPromptSubmit` = short motto only.
...
- **Codex user/memory**, prior chats, or injected **“Memory context”** blocks are often **cross-repo or stale** (foreign dev emails, old tRPC counts, test-debt handoffs from another project). **This file + [`.planning/PROJECT.md`](.planning/PROJECT.md) override them** for stack, people, metrics, and process.
...
## Wiki Knowledge Base (Codex-obsidian)
...
After a GSD phase with a new invariant: append to [`.planning/MEMORY.md`](.planning/MEMORY.md) + optional wiki synthesis page (`/wiki`, `ingest`, `lint the wiki` via Codex-obsidian plugin).
...
Skill paths: `.Codex/skills/impeccable/`; marketing skills symlinked under `.Codex/skills/` from `.agents/skills/`. Full routing: [`.planning/brain/wiki/patterns/ui-skills-routing.md`](.planning/brain/wiki/patterns/ui-skills-routing.md).

**Codex UI hooks (opt-in):** **`[ui]`** / **`[ui-strict]`** — (1) `Skill` tool `frontend-design` *lub* `Read` na `SKILL.md`, (2) `semble search`, (3) analiza i edycje. `[ui-strict]` blokuje edycje plików UI bez 1–2. Wyłącz: prompt bez prefiksu lub `/ui-workflow-off`. Hook paths: `apps/web-vite/`, `apps/landing/`, `packages/ui/` ([`ui-workflow-lib.js`](.Codex/hooks/ui-workflow-lib.js)).
...
- Caveman **full** at start (= `/caveman`); injected via hook. Off: `stop caveman` / `normal mode`. Skill: [`.Codex/skills/caveman/SKILL.md`](.Codex/skills/caveman/SKILL.md).
...
**Delegation (subagent-first):** Prefer Task subagents over ad-hoc bulk shell scripts. Full routing: [`.Codex/skills/cavecrew/SKILL.md`](.Codex/skills/cavecrew/SKILL.md).
