...
- [ ] **FOUND7-03**: `us-east-1` is enabled as a third supported region across the stack — `SUPPORTED_REGIONS`, `DataRegion` enum, `DATABASE_URL_US` env (+ schema), and `feature-flag.ts` region coercion all accept `US` without runtime throw (the 4-place atomic change; precedes any US-data phase)
...
- [ ] **US-FIELD-02**: SSN intake with PII-grade masking (last-4 default; full display behind RBAC `CONTRACTOR_PII:READ`)
- [ ] **US-FIELD-03**: US address validation via USPS Addresses API (CASS-certified format)
...
- [ ] **US-INFRA-01**: A new org with US billing is routed to the `us-east-1` database per the existing per-org region-routing pattern; cross-region read replicas remain off by default
- [ ] **US-INFRA-02**: US-specific R2 storage bucket for tax-form archives (data-residency for US tax records)
- [ ] **US-INFRA-03**: IRS-mandated retention enforced via soft-delete + scheduled archive (4-year 1099-NEC, 7-year backup-withholding records)
