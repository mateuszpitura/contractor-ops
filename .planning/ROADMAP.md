...
**Goal**: OCR pipeline works end-to-end — ClaudeOcrAdapter is registered and getAdapter("claude") resolves at runtime
**Depends on**: Phase 22
**Requirements**: OCR-01
**Gap Closure:** Closes OCR adapter registry gap from v2.0 milestone audit
**Success Criteria** (what must be TRUE):
...
**Goal**: When a workflow starts, tasks with `jiraEnabled: true` in their config automatically create Jira issues via the existing `createJiraIssue` service
**Depends on**: Phase 19 (Jira Integration)
**Requirements**: JIRA-02
**Gap Closure:** Closes JIRA-02 wiring gap from v2.0 milestone audit — service and UI exist but trigger is missing
**Success Criteria** (what must be TRUE):
...
Plans:
- [ ] 25-01-PLAN.md — Add getPortalSigningUrl portalProcedure endpoint, wire EmbeddedSigningModal usePortalAuth prop
...
| 25. Portal E-Sign Auth Fix | v2.0 | 0/1 | Planned | — |
