...
> ⚠️ **POST-MIGRATION PATH NOTICE (2026-05-31).** This doc was written before the web migration on branch `audit/post-migration-parity`. Several file paths below are stale (`apps/web/...`, `packages/api/src/routers/payment.ts`, `routers/approval.ts`, `routers/classification.ts`). The **PLAN.md files (72-01..08) carry the corrected current-tree paths**, and `72-REPLAN-DRIFT-MAP.md` is the authoritative path-correction reference. Where this doc and a PLAN.md disagree on a path, the PLAN.md + drift map win. Key moves: `apps/web` → `apps/web-vite` (Vite SPA, react-i18next, TanStack Router); cron reminders → `apps/cron-worker/src/jobs/handlers/reminders/`; payment router → `routers/finance/payment.ts` (`payment.create` / `payment.lockAndExport`); approval router → `routers/core/approval.ts`; classification router → `routers/compliance/classification.ts`.
