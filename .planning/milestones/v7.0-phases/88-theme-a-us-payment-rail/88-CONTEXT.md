...
- **D-05:** **File export is the always-available default; programmatic is opt-in per-org.** The `ACH_NACHA` file (US-PAY-01) ships and works with no provider creds. Programmatic ACH initiation (US-PAY-03) is opt-in per-org behind the integration framework + the **existing `payments.ach-payouts` PENDING flag** (already registered in `flags-core.ts`/`V7_FLAG_KEYS` per RESEARCH — reuse it, do NOT mint a new flag), dark until live creds — never blocks GA. (Mirrors the 86 IRIS manual-default / A2A-dark posture.)

### USD First-Class + Settlement Currency (US-PAY-02)
- **D-06:** **USD is first-class with a per-org default.** Uses the existing `Organization.defaultCurrency`; USD is treated as a first-class currency (not just a 3-char string). **Correction (RESEARCH F-1):** USD IS already in the ECB feed (`EUR→USD`) and `convertAmount` already cross-rates USD and same-currency-short-circuits — so **no new USD=1.0 short-circuit is needed**; just ensure USD is selectable/first-class and verify the existing FX path handles a USD source/target correctly (add a guard test).
...
- **D-10:** **New `ACH_NACHA` + `FEDWIRE` formats in the payment-export factory**, slotting in like BACS/SWIFT/Elixir/SEPA. Add both to BOTH the `ExportFormat` string union (`payment-format-detection.ts`) AND the Prisma `PaymentExportFormat` enum (they are separate per RESEARCH). **NACHA is hand-rolled** (mirror the dependency-free fixed-width `generateBacsStandard18` — 94-char records, control totals, entry hash, blocking; ODFI fields hand-set; do NOT import a low-trust NACHA dep). **`FEDWIRE` = an ISO 20022 `pacs.008` XML mirroring `generateSwiftXml`** (RESEARCH F-2: the legacy FAIM file was retired 2025-07-14; Fedwire is a bank-channel FedLine message — the export is the pacs.008 the operator hands to their bank, live transmission deferred/adviser-verify). `detectFormat` routes USD + a US bank account → `ACH_NACHA`; high-value above the Same-Day ACH ceiling → `FEDWIRE`. The ceiling is a **config value, not a constant** ($1M now → **$10M on 2027-09-17**). US format selection is gated on `module.us-expansion` + US region.
