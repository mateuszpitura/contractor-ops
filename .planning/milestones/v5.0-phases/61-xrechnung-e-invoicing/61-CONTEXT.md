...
- **D-10:** **Per-org one-time Peppol participant registration via Storecove.**
  ...
  > **AMENDMENT (2026-04-14, during Phase 61 planning):** The original D-10 specified three new fields on `Organization` (`peppolParticipantId`, `peppolParticipantSchemeId`, `peppolParticipantStatus`). During planning the existing `PeppolParticipant` table was reviewed (it already models participant identity + status for the peppol-ae profile) and 61-RESEARCH.md §Collision documented that duplicating these fields on `Organization` would create **dual sources of truth** (every status write would need to touch both places; drift between them would silently break the send-gate).
  >
  > **Reconciled approach — Option A from RESEARCH §Collision:** Extend the existing `PeppolParticipant` table with XRechnung-specific capability columns (`supportsXRechnungCii: Boolean`, `lastCapabilityCheckAt: DateTime?`) instead of adding the three fields to `Organization`. Identity + status continue to live on `PeppolParticipant`; the send-gate (`storecove.transmitInvoice` refuses unless participant is `ACTIVE`) is unchanged in intent but reads from `PeppolParticipant` rather than `Organization`.
  >
  > **User-visible behaviour is identical** to the original D-10: the Settings → E-invoicing page still exposes "Enable Peppol UK", the Storecove webhook still updates participant status, and the send-gate still refuses non-ACTIVE senders. Only the storage location changed.
  >
  > This amendment is implemented by Plan 61-01 (Prisma migration on `PeppolParticipant`) and consumed by Plans 61-05 (capability service) + 61-06 (send mutation + webhook handler). If a future phase needs truly Organization-level Peppol configuration (distinct from per-participant capability), revisit this reconciliation.
