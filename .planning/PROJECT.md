...
## Shipped Milestone: v7.0 GTM Expansion (2026-07-17)

**SHIPPED** — 20 phases (82–101), audit-passed. US cross-border tax + payment rail (region routing, W-9/W-8 + tax-treaty engine, TIN-match → 1099-NEC → IRIS e-file → state filing, 1042-S + US classification, USD ACH/Fedwire), six-market workforce management (Worker abstraction + EmployeeProfile ×6, akta/personnel file, leave + KP-grade time incl. UAE/KSA statutory rules, on/offboarding, payroll export adapters, HRIS two-way sync, employee portal, HR dashboard), and an open integration platform (versioned public REST API + SDKs, API keys/scopes/rate-limits, signed outbound webhooks + OWASP gate, Zapier/n8n/Make marketplace + developer portal + sandbox tier). Close-out audit found and fixed 2 latent built-but-unwired producer seams (outbound-webhook emit, TIN-match trigger) before archive. Deferred: human-QA/adviser-verify on US tax-form flows (84/85), v7.5 items (e-ZLA/eAU/HMRC-RTI-direct), ADP native (v7.1), external marketplace submissions (EXTERNAL-ENABLEMENT). Full record: `.planning/milestones/v7.0-ROADMAP.md` + `.planning/MILESTONES.md` + `.planning/milestones/v7.0-MILESTONE-AUDIT.md`. LOCAL-ONLY deploy posture + DEFERRED legal sign-off preserved.

<details>
<summary>Original v7.0 goal (at milestone start)</summary>
...
**Locked decisions (2026-05-31):** themes run parallel (`WORKER-01` the only hard serialization point); `Workforce` + `US Cross-Border` add-on SKUs (`requireAddOn` middleware), Theme C tier-gated within base; 3 US payroll adapters; 10 US states + free-text fallback; e-ZLA / eAU / HMRC RTI deferred to v7.5; LOCAL-ONLY deploy posture + DEFERRED legal sign-off carry forward. Full backlog: `.planning/milestones/v7.0-BACKLOG.md`. Concrete requirements → archived `.planning/milestones/v7.0-REQUIREMENTS.md`.

</details>

## Current State

**v7.0 GTM Expansion SHIPPED 2026-07-17** — 20 phases (82–101), all SUMMARYs complete; milestone audit PASSED. Three parallel themes delivered: **A** US cross-border (region routing → us-region client/bucket, W-9/W-8 + tax-treaty engine feeding 1042-S withholding + payment basis + portal W-8 claim, TIN-match → 1099-NEC → IRIS A2A e-file → state filing, 1042-S + US worker classification, USD ACH-NACHA/Fedwire rail); **B** workforce (Worker discriminated abstraction + EmployeeProfile ×6 markets, akta/Personalakte INSERT-only ewidencja, append-only leave ledger + KP-grade working-time registry incl. the UAE/KSA statutory rules added at close, on/offboarding, payroll export profile-registry + Gusto/QuickBooks native, Personio/BambooHR two-way sync with source-of-truth split, employee self-service portal, HR dashboard composing the compliance-expiry + saudization engines); **C** integration platform (versioned public REST API with the P99 api-key actor model filling write-half FKs, keys/scopes/rate-limits, signed outbound webhooks behind a GREEN OWASP API Top-10 gate, generated Zapier/n8n/Make apps + Scalar developer portal + free sandbox tier). The close-out `integration-checker` audit caught and fixed two latent **built-but-unwired producer seams** before archive — the outbound-webhook emit (`enqueueWebhookEvent` now fired from all 16 domain mutations in-tx) and the TIN-match/backup-withholding trigger — plus reconciled 106 requirements to Complete. Deferred (documented): human-QA/adviser-verify on the US tax-form wizards (84/85, post-deploy), v7.5 items (e-ZLA/eAU/HMRC-RTI-direct), ADP native (v7.1), and external marketplace submissions (EXTERNAL-ENABLEMENT). Full record: `.planning/milestones/v7.0-{ROADMAP,REQUIREMENTS,MILESTONE-AUDIT}.md`; close-time deferred items in `STATE.md`. LOCAL-ONLY deploy posture + DEFERRED legal sign-off preserved.
...
---
*Last updated: 2026-07-17 — v7.0 GTM Expansion SHIPPED (20 phases 82–101, audit passed, 2 close-out producer-seam gaps fixed). Next: `/gsd:new-milestone`.*
