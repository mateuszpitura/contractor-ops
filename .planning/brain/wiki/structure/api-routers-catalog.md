...
| `employeeLifecycle` | employee on/offboarding composition seam (`employee-lifecycle-router.ts`), all `employee`-RBAC-gated + `assertWorkforceEnabled` + `resourceType:'EMPLOYEE'` audit: `get` (`employee:read` — displayName/employmentStatus/`terminatedAt` for the UI IdP gate), `startOnboarding`/`startOffboarding` (`employee:update` — resolve the ACTIVE per-market `WorkflowTemplate` by `mapCountryCodeToJurisdiction`, start a worker run via the shared `startWorkflowRun` helper — never a duplicate `workflowRun.create`), `recordTermination` (writes `EmployeeProfile.terminatedAt` + status TERMINATED — the dated signal the IdP cooldown reads), `generateCert` (creates a DRAFT `StatutoryCertificate` + `renderAndArchiveStatutoryCert` → signed URL). Full flow: [[domains/worker-onboarding-offboarding]] |
...
| `payrollExport` | per-market payroll export adapters (`payroll-export-router.ts`), `employee:read`-gated + `assertWorkforceEnabled` + per-target `evaluate(payroll.*)` FORBIDDEN gate: `listTargets` (the 10 registered targets, each annotated with the org's per-adapter flag state), `export` (**also** `employeePii:read` — unmasked feed export; `.strict()` `{targetId, employeeIds, format?}` → `buildPayrollFeed` → `engine.generate` → `writeAuditLog('payroll.export', resourceType:'EMPLOYEE')` → `fileBase64`), `connectNative` (Gusto/QuickBooks OAuth entry point). Gusto/QuickBooks native push is flag-deferred (CSV is the shipping path). Full model + flow: [[domains/payroll-export]] |
...
| `portalEmployee` | employee self-service (dark behind `module.employee-portal`, spread via `isEmployeePortalRegistered()`): `getDashboard`, `getLeaveBalance`, `listMyLeaveRequests`, `getMyTime`, `getMyEwidencja`, `submitTimeOffRequest`, `getMyAkta`, `getMyAktaDocumentUrl`, `getPayStubAvailability`. Every read/write is scoped to the SESSION worker (`ctx.workerId` from `portalEmployeeProcedure`) — no procedure accepts a client `workerId`; the self-reads are `.strict()` so a smuggled `workerId` is a hard rejection. `submitTimeOffRequest` reuses the shared leave services (`computeLeaveBalance` + `routeToLeaveChain`/`createApprovalFlow`) and writes an `EMPLOYEE`-actor audit row in-tx. `getMyAkta` (`portal-employee-akta.ts`) returns the caller's OWN personnel file filtered to `PERSONNEL_FILE_SELF_VIEW_SECTIONS` (section C / pay+PII excluded) — the allowlist is applied in the document `where`, so an excluded section's rows never load; `getMyAktaDocumentUrl` re-checks own-file + allowlist before signing a regional download URL; `getPayStubAvailability` returns a truthful `{ available:false, reason:'EXTERNAL_PAYROLL' }` (v7.0 ships no payslip surface — payroll is export-only). |
| `portalManager` | employee-portal MANAGER surface (`portal-manager-router.ts`; same dark-mount + `module.employee-portal` gate): `getTeamOverview`, `listReportLeaveRequests`, `approveReportLeaveRequest`, `rejectReportLeaveRequest`. Gated on `portalManagerProcedure` (admits only a caller with ≥1 report). Direct reports are resolved SERVER-SIDE from the reporting-line edge (`EmployeeProfile.managerWorkerId = ctx.workerId`, same org) via `services/portal-reports.ts` — report ids are never taken from client input; every mutation re-derives the request's own `workerId` and calls `assertIsDirectReport` BEFORE acting. Approve reuses the shared `finalizeApprovedLeave` transition (status flip + DEDUCTION ledger + balance-cache refresh); reject marks REJECTED with no ledger movement — both write an `EMPLOYEE`-actor audit row in-tx. |

The `portalEmployee` and `portalManager` namespaces are dark-mounted like the staff `conditionalWorkforceRouters` (absent → `METHOD_NOT_FOUND` when the flag is unregistered at boot); `portalEmployeeProcedure` / `portalManagerProcedure` re-assert `module.employee-portal` (and, for manager, ≥1 report) per request (FORBIDDEN when dark).
...
`packages/api/src/routers/public-api/*` (`publicApiRouter`) — read procedures for 9 entities + **write** procedures for 6 (contractor/invoice/payment/paymentRun/workflow/workflowTask), each under `apiKeyTenantProcedure` with a mandatory `requirePermission` scope. Phase 100 passed the OWASP gate and **un-hid** the write routes (now in the spec/SDK); the per-org `module.public-api` flag gate still 404s a non-granted org. `_initiatePayoutForRun` stays deferred. Full surface: [[domains/public-api-surface]].
