...
| `portal/employee/` | [[domains/employee-portal]] — employee self-service (dashboard, leave, time, documents, pay); hooks under `components/portal/employee/hooks/` are the sole tRPC boundary → `portalEmployee.*`; the time-off dialog posts `submitTimeOffRequest` with no workerId; a dark surface degrades via `isModuleDarkError` |
| `portal/employee/team/` | [[domains/employee-portal]] — manager `/team` overview + leave approvals; `components/portal/employee/team/hooks/use-manager-*.ts` → `portalManager.*`; a non-manager (FORBIDDEN) renders a forbidden state |
...
| `payroll/` | [[domains/payroll-export]] — per-market payroll export surface; `payroll-export-page.tsx` (thin flag-gated composer) at `/payroll-export`, render-tree-removed when `useFlag('module.workforce-employees')` is off; `payroll-export-container.tsx` (loading/empty/error + worker-id entry) → `hooks/use-payroll-export.ts` (sole tRPC boundary — `payrollExport.listTargets`/`export`, base64→blob download) → presentational `payroll-export-panel.tsx` (target list + per-adapter enablement + CSV/XML actions). `PayrollExport` i18n en/de/pl/ar/en-US |
