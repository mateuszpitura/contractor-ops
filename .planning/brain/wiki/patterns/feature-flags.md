...
| HR-dashboard gate | `module.hr-dashboard` (category `module`, owner `workforce-platform`, default OFF, signoff PENDING→APPROVED post-deploy) — gates the staff `dashboard/hr` HR command surface (headcount / vacation-utilization / document-expiry / probation-watchlist / Gulf-nationalisation widgets); **layered on** `module.workforce-employees` (the data prerequisite). Gated via the `module.hr-` namespace prefix (signoff-registry entry required); NOT part of the v7.0 GTM cohort, so `V7_FLAG_KEYS` stays at 20. Re-asserted per request in the `hrDashboard` router. See [[domains/hr-dashboard]] |
...
| Developer-experience gates (Theme C) | `module.developer-portal` (the `/v1/docs/*` portal + `/v1/collections/*`), `module.public-status-page` (the `/v1/status.json` aggregator + landing `/status`), `module.api-sandbox` (external self-serve `co_test_` key issuance) — all category `module`, default OFF, **gated** (PENDING in the signoff registry, exact-key prefixes in `GATED_FLAG_NAMESPACE_PREFIXES`). Each surface 404s / no-ops when dark. See [[domains/developer-experience]] |
