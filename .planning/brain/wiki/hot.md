...
## Sentry PII scrub = one shared keyword list

The four `sentry-scrub.ts` `beforeSend` copies (`apps/api`, `apps/public-api`, `apps/cron-worker`, `apps/web-vite` — differ only in `@sentry/node` vs `@sentry/react`) import `isPiiScrubKey` / `PII_SCRUB_KEYWORDS` from the dependency-free `@contractor-ops/logger/pii-mask` subpath — **no local hand-lists** (they had drifted, missing ssn/ein/pesel/iqama/emiratesid/nationalid + bank/DOB). Add a redaction keyword there, aligned with `PII_MASK_PATHS` (pino redact), never in a copy. Substring match → short tokens broad by design (`ein` also hits `eInvoiceId`; over-redacting crash reports is the safe side). Guard: `packages/logger/src/__tests__/pii-scrub.test.ts`. Detail: [[integrations/sentry]] · [[patterns/logging-and-errors]].
