---
title: Wiki index
type: index
updated: 2026-06-10
---

# Wiki index — contractor-ops

Read [[hot]] first. Code symbols: **semble** — never wiki alone. Discovery: [[meta/agent-discovery]].

## Meta

- [[overview]]
- [[meta/agent-discovery]]
- [[meta/page-template]]
- [[meta/refresh-triggers]]
- [[meta/retrieval]]
- [[meta/graphify]]
- [[meta/obsidian-setup]]
- [[meta/dashboard]] — navigation hub; [[meta/wiki-tables]] — Base table views
- [[log]]

## Structure (repo compass)

- [[structure/_index]]
- [[structure/monorepo-topology]]
- [[structure/apps]]
- [[structure/packages]]
- [[structure/api-router-groups]]
- [[structure/api-routers-catalog]]
- [[structure/web-vite-domains]]
- [[structure/prisma-schema-areas]]
- [[structure/cron-jobs]]
- [[structure/cms-and-landing]]
- [[structure/key-services]]

## Patterns (CI invariants)

- [[patterns/_index]]
- [[patterns/web-vite-data-layer]]
- [[patterns/tenant-and-audit]]
- [[patterns/entity-id-and-money]]
- [[patterns/trpc-procedure-stack]]
- [[patterns/portal-auth]]
- [[patterns/feature-flags]]
- [[patterns/rbac-permissions]]
- [[patterns/validators-boundaries]]
- [[patterns/logging-and-errors]]
- [[patterns/multi-region-db]]
- [[patterns/testing-and-msw]]
- [[patterns/data-tables-workbench]]
- [[patterns/agent-delegation]]
- [[patterns/ci-guards]]
- [[patterns/i18n-and-locales]]
- [[patterns/better-auth-staff]]
- [[patterns/audit-log]]

## Domains (business flows)

- [[domains/_index]]
- [[domains/invoice-to-payment]]
- [[domains/portal-external]]
- [[domains/contractors-engagements]]
- [[domains/worker-foundation]]
- [[domains/contracts-lifecycle]]
- [[domains/documents-and-ocr]]
- [[domains/notifications-and-reminders]]
- [[domains/approvals-engine]]
- [[domains/workflows-and-roles]]
- [[domains/payments-and-bank-files]]
- [[domains/us-payment-rail]]
- [[domains/compliance-dashboard]]
- [[domains/classification-ir35]]
- [[domains/equipment-logistics]]
- [[domains/time-and-reconciliation]]
- [[domains/billing-and-feature-gates]]
- [[domains/onboarding-and-import]]
- [[domains/settings-and-org-admin]]
- [[domains/search-and-reports]]
- [[domains/staff-dashboard]]
- [[domains/consent-gdpr-pdpl]]
- [[domains/gulf-saudization]]
- [[domains/tax-and-wht]]
- [[domains/idp-deprovisioning]]
- [[domains/public-api-surface]]

## Integrations

- [[integrations/_index]]
- [[integrations/framework-core]]
- [[integrations/modern-treasury]]
- [[integrations/plaid]]
- [[integrations/ksef]]
- [[integrations/peppol]]
- [[integrations/zatca]]
- [[integrations/einvoice-profiles]]
- [[integrations/google-workspace]]
- [[integrations/jira]]
- [[integrations/linear]]
- [[integrations/teams]]
- [[integrations/slack]]
- [[integrations/entra-okta-github]]
- [[integrations/docusign-esign]]
- [[integrations/stripe-billing]]
- [[integrations/qstash-cron]]
- [[integrations/neon-r2]]
- [[integrations/infisical-secrets]]
- [[integrations/unleash-flags]]
- [[integrations/sentry]]
- [[integrations/couriers]]
- [[integrations/gov-api]]

## Decisions & risks

- [[decisions/_index]]
- [[decisions/memory-authority]]
- [[decisions/arch-decisions]]
- [[decisions/tech-debt-hotspots]]

## Raw source summaries

- [[sources/_index]]

## External canonical (link out)

| Layer | Path |
|-------|------|
| Bootstrap | `.planning/INDEX.md` |
| Code maps | `.planning/codebase/` |
| Intel | `.planning/intel/` |
| Graph | `.planning/graphs/graph.json` |
| Invariants | `.planning/MEMORY.md` |
