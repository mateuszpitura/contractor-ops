...
When behavior varies by **provider**, **jurisdiction**, or **job type**, prefer the repo’s register/get registry over `switch` in routers or services.

| Signal | Action |
|--------|--------|
| 3+ variants, same interface | `registerX` + `getX(id)` in the owning package |
| New provider / jurisdiction file | Side-effect `register` on import (see `compliance-policy/policies/de.ts`) |
| Per-request credentials (IdP, OAuth) | **Factory** new adapter instance — do not mutate registry singleton under concurrency |
| Tests | Expose `clearX()` and reset in `beforeEach` |

Canonical list: [`.planning/brain/wiki/patterns/registry-plugin.md`](../brain/wiki/patterns/registry-plugin.md).
