...
- **Global scope**: `$HOME/Repos/projects/contractor-ops/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `$HOME/Repos/projects/contractor-ops/.claude/notes/`)
...
2. Glob `$HOME/Repos/projects/contractor-ops/.claude/notes/*.md` (if directory exists) — global notes
...
Global ($HOME/Repos/projects/contractor-ops/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `$HOME/Repos/projects/contractor-ops/.claude/notes/` — works in any directory
